            ...
            <div tabIndex={0} className="dropdown-content pt-2 font-noto_sans">
                ...
                { !userData.isLoggedIn ? (
                    <ul tabIndex={0} className="bg-darkpurple rounded-box z-[1] w-60 p-4 shadow">
                        <li key='li-element-6' onClick={() => {ModalToggle('register')}} className="px-4 h-7 rounded-3xl bg-orangered text-white text-base flex items-center justify-center font-medium cursor-pointer btn hover:bg-orangered border-0">Registrar</li>
        
                        <li key='li-element-7' onClick={() => {ModalToggle('login')}} className="px-4 h-7 rounded-3xl bg-white/10 text-white text-base flex items-center justify-center font-medium cursor-pointer mt-2 btn hover:bg-white/10 border-0">Entrar</li>
                    </ul>
                ) : (
                    <div className="bg-darkpurple rounded-box overflow-hidden">
                        { userData.name ? (
                            <div className="xl:hidden p-4 hover:bg-white/10 cursor-pointer">
                                ...
                                <div onClick={openProfileModal} className="flex flex-row items-center gap-x-2 text-neutral-500 text-[15px]">
                                    <FaPencil className=""/>
                                    <p>Editar perfil</p>
                                </div>
                            </div>
                        ...
                        <ul className="w-full flex flex-col items-start *:px-6 *:whitespace-nowrap *:gap-x-3 *:border-none *:outline-none font-medium text-base">
                            ...
                            <li onClick={signOutUser} key='li-element-13' className="text-error hover:bg-red-950 hover:text-white cursor-pointer h-12 w-full flex items-center flex-row-reverse xl:flex-row justify-end xl:justify-start">
                                Sair
                                <BsDoorOpenFill className="text-lg"/>
                            </li>

                            <li key='li-element-14' onClick={deleteCurrentUser} className="text-error hover:bg-red-950 hover:text-white cursor-pointer h-12 w-full flex items-center flex-row-reverse xl:flex-row justify-end xl:justify-start">
                                Excluir conta
