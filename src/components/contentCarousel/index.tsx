...
export default function ContentCarousel( props: carouselProps ) {
    ...
    const { fetchMoviesByGenre, fetchReleasedMovies, fetchSeriesByGenre, fetchReleasedSeries } = useTmdbFetch();
    ...
    useEffect(() => {
       ...
       if ( props.contentType === 'movie' ) {
            if ( props.contentGenre !== 'release' ) {
                handleFetchResponse(fetchMoviesByGenre( props.contentGenre, props.pageNumber ));
            } else {
                handleFetchResponse(fetchReleasedMovies( props.pageNumber ));
            }
       } else {
            if ( props.contentGenre !== 'release' ) {
                handleFetchResponse(fetchSeriesByGenre( props.contentGenre, props.pageNumber ));
            } else {
