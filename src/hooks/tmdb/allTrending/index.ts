...
export const fetchAllTrending = async (
    ...
    try {
        const res = await fetch(`https://api.themoviedb.org/3/trending/${type}/day?api_key=${token}&language=${lang}&include_image_language=${region},en,null&page=${page}`, {
            cache: page === 1 ? 'force-cache' : 'no-store',
            next: { revalidate: page === 1 && 3600 } // 1 hora
        });

        if (!res.ok) {
            throw new Error(`Erro ao buscar series em alta: ${res.status}`);
        };
